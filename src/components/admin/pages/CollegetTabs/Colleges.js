...
const fetchColleges = ()=>{
  axios.get(`${BASE_URL}/api/college`).then((res)=>{setData(res.data.responses)}).catch((err)=>{console.log(err);})
}
useEffect(()=>fetchColleges(),[])
 ...
 <Paper sx={{p:"20px",width:"100%",m:"20px"}} elevation={2} >
      ...
      <Select onClick={(event)=>event.stopPropagation()} size='small'  labelId='mylabel'  sx={{width:{lg:"120%",md:"100%"},color:"black"}} value={dropdown2Value} onChange={handleDropdown2Change}>
        <TextField fullWidth focused={false} autoFocus size='small' id="searchInputadmin" label="Search" variant="outlined" />
        <MenuItem sx={{fontSize:"10px"}} disabled value="">All users</MenuItem>
        <MenuItem sx={{fontSize:"10px"}} value="College Padho">College Padho</MenuItem>
        <MenuItem sx={{fontSize:"10px"}} value="Rohit">Rohit</MenuItem>
        <MenuItem sx={{fontSize:"10px"}} value="College Padho">College Padho</MenuItem>
        <MenuItem sx={{fontSize:"10px"}} value="Rohit">Rohit</MenuItem>
        <MenuItem sx={{fontSize:"10px"}} value="College Padho">College Padho</MenuItem>
        <MenuItem sx={{fontSize:"10px"}} value="Rohit">Rohit</MenuItem>
    ...
    <Grid item xs={12} lg={12} md={12} sm={12}>
          <TableMain dummyData={data} rows={rows} setRows={setRows}/>
    </Grid>
      </Grid>
    </Paper>
              </Grid>
